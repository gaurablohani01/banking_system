    ...
    try:
        if request.method=="POST":
            ...
            if not User.objects.filter(phone_number= phone_number).exists():
                    ...
                    if not User.objects.filter(email = email).exists():
                        if not first_name.lower() in password1.lower() or last_name.lower() in password1.lower(): 
                            if password1 == password2:
                                user= User.objects.create(first_name= first_name, last_name=last_name, phone_number=phone_number, username=username, email=email)
                ...
                messages.warning(request, f'Phone Number already exists')
                return redirect('register')
        context={
            'title': "Register"
        }
